...
$start = $page * $per_page;
 ...
 $result1 = $scad->getSpecl($alpha);
 $result = $scad->getSpecl1($alpha,$start,$per_page);
 $end  = count($result1);
         ...
         for($i=0;$i<$count;$i++)
		 {
		  ...
		  $msg .= '<li style="width:372px;color:#2a9cdc;cursor:pointer" class="spcl_foot1" value="'. $result[$i]['id'].'" id="'. $result[$i]['speciality_id'].'">
   '. $result[$i]['name'].'
</li>';
